...
find_task() { find "$Q" -name "$1.json" 2>/dev/null | head -1; }
...
case "${1:-list}" in
  ...
  move)
    id="$2"; dest="$3"; f="$(find_task "$id")"
    [ -z "$f" ] && { log ERROR "task.sh: $id not found"; exit 1; }
    case "$dest" in pending|active|completed|failed) ;; *) log ERROR "bad state $dest"; exit 1 ;; esac
  ...
  show)
    f="$(find_task "$2")"; [ -z "$f" ] && { echo "not found"; exit 1; }; cat "$f" ;;
